...
docker run --name pihr -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=Helloworld1?' -e 'MSSQL_PID=Developer' -p 1433:1433 --platform linux/amd64 -d mcr.microsoft.com/mssql/server:2022-latest

sudo docker exec -it pihr mkdir var/opt/mssql/backup

cd ~/Downloads/

sudo docker cp huduri_production20240930.bak pihr:var/opt/mssql/backup

docker exec -it --user root pihr /bin/bash
