...
async def query_fixer(query: str, planList: PlanList, queryList: QuerySteps, error_message: str):
    ...
    => I did the following query: {queryList.steps[queryList.len - 1].sql_query}
    => But, I got this error while running the final query: {error_message}
    => please fix the final query and return a query which is correct and also reaches the goal.
    """
    
    get_llm_logger().info(f"FIXING QUERY")
    
    completion = client.beta.chat.completions.parse(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You are a sql query fixer. You will be given a list of query with a query plan and a error message while running the last and final combined query. Fix the final query to reach the goal and return in the query according to the response format ."},
